...
from exception.exceptions import PathologyBaseException, LLMInvocationException, LLMResponseParseException
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    @app.exception_handler(PathologyBaseException)
    async def pathology_exception_handler(_request: Request, exc: PathologyBaseException) -> JSONResponse:
        logger.error("PathologyBaseException caught | code: %s | message: %s", exc.error_code, exc.message)
        response = GenericResponse.failure(error_code=exc.error_code, error_message=exc.message)
