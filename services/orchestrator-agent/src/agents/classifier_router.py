# 4-tier hybrid classifier: Rule → BioBERT zero-shot NLI → fine-tuned ClinicalBERT → LLM fallback.
# Each tier escalates only when confidence is below its threshold.
# secondary_check_needed is determined by keyword rule at every tier.
...
# Tier 1 keyword banks — mirror the specialist descriptions in _CLASSIFIER_SYSTEM.
...
async def route_symptoms(symptoms: str) -> tuple[str, bool, str]:
    """Route through the 4-tier cascade. Returns (specialist, secondary_check_needed, reasoning)."""
    low = symptoms.lower()
