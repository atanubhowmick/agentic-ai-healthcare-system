...
from exception.exceptions import CardiologyBaseException, LLMInvocationException, LLMResponseParseException
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    @app.exception_handler(CardiologyBaseException)
    async def cardiology_exception_handler(_request: Request, exc: CardiologyBaseException) -> JSONResponse:
        logger.error("CardiologyBaseException caught | code: %s | message: %s", exc.error_code, exc.message)
        response = GenericResponse.failure(error_code=exc.error_code, error_message=exc.message)
