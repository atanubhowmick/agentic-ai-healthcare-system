# Searches the ChromaDB MIMIC-IV oncology collection and returns the top-k most similar
...
def search_similar_cases(symptoms: str, top_k: int = 3) -> List[dict]:
    """Return the top-k MIMIC-IV oncology cases most similar to the given symptoms.
    Returns an empty list if the collection is unavailable or no results are found."""
    col = _get_collection()
    if col is None:
        return []
