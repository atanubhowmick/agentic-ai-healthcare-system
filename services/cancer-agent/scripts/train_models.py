# Train TF-IDF + classifier models from MongoDB (mimic_iv_records) and save them
# to a pickle file for use by the XAI validation service's SHAP explainer.
#
# Models trained:
...
#
# Usage:
#   python train_models.py [output_path]
#
...
def train_models(output_path: str) -> None:
    print(f"[TRAIN] Training models from MongoDB (this may take a few minutes)...")
    ...
    if _predictor._clf_sev is not None:
        ...
        print(f"[TRAIN] severity model ready | "
    ...
    if _predictor._clf_emg is not None:
        ...
        print(f"[TRAIN] emergency model ready | threshold={_predictor._thresh_emg:.4f}")
    ...
    if _predictor._clf_hosp is not None:
        ...
        print(f"[TRAIN] hospitalization model ready | threshold={_predictor._thresh_hosp:.4f}")
    ...
    if _predictor._clf_can is not None:
        ...
        print(f"[TRAIN] cancer_type model ready")

    if not models:
        print("[TRAIN] ERROR: No models were trained — is MongoDB populated?")
        sys.exit(1)
    ...
    print(f"\n[TRAIN | EXPORT] Saved {len(models)} models → {output_path} ({size_mb:.1f} MB)")
    print(f"[TRAIN | EXPORT] XAI validation service will load from: {output_path}")
